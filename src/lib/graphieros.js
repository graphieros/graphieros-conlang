...
function linear({
    ...
    let GLYPH_INSTANCE_COUNTER = 0;

    function mySplit(value, splitter) {
        return value.split(splitter);
    }

    const concat = (string0, string1) => `${string1}${string0}`;

    const convertString = (sequenceValue, split0, split1, conc) => {
        return mySplit(sequenceValue, split0)
            .map((element) => mySplit(element, split1))
            .map((array) => array.map((element) => concat(element, conc)));
    };

    const getOneGlyphCords = (glyph, database, incrY, incrXValue, oddIncr) => {
        INCR += ADDER;

        return database
            .filter((element) => element.name === glyph)
            .map((entry, k) => {
                const paths = (k === 0
                    ? entry.path.map((cords) =>
                        cords.map((cord, i) =>
                            i % 2 === 1 ? cord + incrY + oddIncr : cord + incrXValue
                        )
                    )
                    : entry.path.map((cords) =>
                        cords.map((cord, i) =>
                            i % 2 === 1 ? cord + incrY : cord + incrXValue
                        )
                    )
                );

                paths.glyphName = glyph;
                return paths;
            });
    };

    const getAllCords = (array, database, incrXValue, oddIncr) => {
        return array.map((arr) => arr.map((element) => getOneGlyphCords(element, database, INCR, incrXValue, oddIncr)));
    };

    const populateLinkPaths = (array) => {
        return array
            .map((element) => `<path class="link" d="M ${element}" />`)
            .toString()
            .replaceAll(",", " ");
    };
    ...
    const getWordCountAndLength = (value) => {
        const output = [];
        mySplit(value, " ").forEach((split) => output.push(mySplit(split, "-").length));
        return output;
    };

    const buildLinks = (array, incrXValue, oddIncr) => {
        const output = [];
        let Y = 328;
        ...
        array.forEach((element) => {
            if (element > 1) {
                output.push([
                    ...
                    Y + oddIncr,
                    X + incrXValue,
                    Y + oddIncr + ((element - 1) * GLYPH_HEIGHT) + ((element - 1) * SPACE)
                ]);
                Y += (element * GLYPH_HEIGHT) + (element * SPACE);
            } else {
                Y += (element * GLYPH_HEIGHT) + (element * SPACE);
            }
        });
    ...
    const renderGlyphs = (seq, incrXValue, oddIncr, lineIndex) => {
        const DOM_render_glyphs = getAllCords(
            convertString(seq, " ", "-", "_"),
            graphieros_dictionnary,
            incrXValue,
            oddIncr
        );

        const DOM_links = buildLinks(getWordCountAndLength(seq), incrXValue, oddIncr);
        DOM_paths.push(populateLinkPaths(DOM_links));

        DOM_render_glyphs.forEach((level0) => {
            level0.forEach((level1) => {
                level1.forEach((level2, wordIndex) => {
                    const instanceId = `l${lineIndex}-g${GLYPH_INSTANCE_COUNTER++}`;

                    DOM_paths.push(
                        populateGlyphPaths(level2, {
                            instanceId,
                            glyph: level2.glyphName,
                            line: incrXValue,
                            word: wordIndex
                        })
                    );
    ...
    for (let i = 0; i < DOM_lines.length; i += 1) {
        ...
        renderGlyphs(DOM_lines[i], incrementX, oddIncr, i);

        incrementX += 170;
        INCR = 200;

        const wordsHeights = getWordCountAndLength(DOM_lines[i]).reduce((n0, n1) => n0 + n1);
    ...
    // Centering
    const lineCount = DOM_lines.length;
    const viewBoxWidth = SVG_WIDTH * lineCount;
    const drawnWidth = SVG_WIDTH + ((lineCount - 1) * 170);
    const translateX = (viewBoxWidth - drawnWidth) / 2;

    SVG.setAttributeNS(
        null,
        "viewBox",
        `0 0 ${viewBoxWidth} ${maxHeight * (GLYPH_HEIGHT + SPACE) + (unique ? 450 : 550)}`
    );
    SVG.setAttributeNS(null, "height", glyphWidth);
    SVG.setAttributeNS(null, "width", glyphWidth);

    GROUP.setAttributeNS(null, "fill", "none");
    GROUP.setAttributeNS(null, "stroke-linejoin", "round");
    GROUP.setAttributeNS(null, "stroke-linecap", "round");
    GROUP.setAttributeNS(null, "stroke-width", STROKE_WIDTH);
    GROUP.setAttributeNS(null, "stroke", `rgb(${colors})`);
    GROUP.setAttributeNS(null, "transform", `translate(${translateX}, 0)`);

    GROUP.innerHTML = DOM_paths;
...
export { a, q, w, l, lm, m, r, rm, graphieros_dictionnary, linear, fractal, molecular, callifractal };